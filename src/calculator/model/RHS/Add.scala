...
class Add(calculator: Calculator) extends State(calculator) {
  ...
  override def numberPressed(number: Int): Unit = {
    calculator.state = new NumberPressed(calculator, number, new Add(calculator))
  }
  ...
  override def decimalPressed(): Unit = {
    calculator.state = new DecimalRHS(calculator, new Add(calculator))
  }
